...
def _fit_territory(terr_df: pd.DataFrame, channels: list, config: dict, freq: str) -> dict:
    ...
    # Apply adstock + saturation in-memory (no file I/O)
    for ch in channels:
    ...
    # Prior-contribution floor for zero-coefficient channels
    ...
    for i, ch in enumerate(channels):
        ...
        avg_sat       = terr_df[f"{ch}_saturated"].values.mean()
        ...
        channel_results[ch] = {
            ...
            "avg_period_spend_k": round(float(raw_spend.mean()), 1),
...
def run_geo_ols_mmm_tool(data_path: str, config_path: str, freq: str = "weekly") -> str:
    ...
    try:
        ...
        for terr_key in sorted(df["territory"].unique()):
            ...
            all_results["territories"][terr_key] = {
                "label": label,
                **res,
            }
